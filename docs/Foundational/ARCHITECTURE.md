...
| v0.080 | Android backend -- `arklight android` packages a `build-dir` into a native Android project via `androidx.webkit.WebViewAssetLoader`, evolving the existing `ARKlight-Viewer-for-Android-Devices` app into the backend's runtime (staged `scaffold` -> CI build (2a) -> CI install/launch smoke test (3a) -> local `build` (2b) -> `--install` (3b) -> `--release` CLI ladder); design complete in `docs/DESIGN-NOTES.md`, staged implementation tracked in `docs/Backends/ANDROID-BACKEND-IMPLEMENTATION.md`, Stages 0, 1, 2a, and 3a (`arklight android scaffold`, including its generated GitHub Actions CI build + emulator smoke-test workflow) done | IN PROGRESS |
