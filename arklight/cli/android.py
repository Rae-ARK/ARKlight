...
on GitHub-hosted runners on every push/PR (Stage 2a), then installs
and launches it on a throwaway emulator on that same runner to confirm
it doesn't crash immediately on startup (Stage 3a). Both are CI-only
build/install verification, no local toolchain required, folded into
this same command rather than separate subcommands since they cost
nothing to generate. `arklight android build` (Stage 2b -- shells out
to a *local* `./gradlew assembleDebug`, requires a JDK on this
machine) and `arklight android build --install` (Stage 3b -- `adb
install` onto a device/emulator the user has actually connected) are
