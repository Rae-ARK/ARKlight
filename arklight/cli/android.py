...
`runtime._github_ci_workflow_yml`) that builds a debug APK on
GitHub-hosted runners on every push/PR. That's Stage 2a of the design
doc's CLI ladder: CI-only build verification, no local toolchain
required, folded into this same command rather than a separate
subcommand since it costs nothing to generate. `arklight android
build` (Stage 2b -- shells out to a *local* `./gradlew
assembleDebug`, requires a JDK on this machine) is a separate,
not-yet-implemented command; this module only produces the project
shell, it never invokes Gradle itself.
"""
