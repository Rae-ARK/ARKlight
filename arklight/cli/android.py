...
`runtime._github_ci_workflow_yml`) with three jobs: builds a debug APK
on GitHub-hosted runners on every push/PR (Stage 2), installs and
launches it on a throwaway emulator on that same runner to confirm it
doesn't crash immediately on startup (Stage 3), and separately builds
a release APK, signed if the repo has keystore secrets configured and
unsigned otherwise (Stage 4). All three are CI-only build/install
verification, no local toolchain required, folded into this same
command rather than separate subcommands since they cost nothing to
generate. `arklight android build` (Stage 5 -- shells out to a
