...
generation logic by accretion the way the old monolithic `render.py`
did. Anything that *computes* CSS (design tokens, custom classes)
lives in its own sibling module instead of here.

`BASE_CSS_HEADER` is the file's leading comment; `BASE_CSS_BODY` is
every rule for the default tag mapping and utility classes. The
`:root`/`@property` block is deliberately NOT here -- it's generated,
not static, and lives in `design_tokens.py`.
"""
