...
def _build_runtime_js(ir: WebsiteIR) -> str:
    ...
    parts: list[str] = [
        ...
        "// shipped modifier-parsing runtime function), and htmx-3",
        "// (Action.* dispatch wires through a single delegated click",
        "// listener instead of a per-element wiring pass). Implements",
        "// only the named behaviors and Action.*(...) references this site",
        "// actually uses -- see arklight.ir.schema.BEHAVIOR_REGISTRY /",
        "// ACTION_REGISTRY / MODIFIER_REGISTRY. No other JavaScript runs",
        "// on this site. Pages with state also carry a vendored snabbdom",
        "// core (init + h, no optional modules) -- see",
    ...
    if has_state:
        ...
        ready_calls.append("    wireActionInterceptor(store);")
