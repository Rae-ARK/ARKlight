...
def _build_runtime_js(ir: WebsiteIR) -> str:
    ...
    parts: list[str] = [
        ...
        "// hx-on:click instead of a hand-rolled wiring pass) and htmx-2",
        "// (Action.* event modifiers compile to hx-trigger instead of a",
        "// shipped modifier-parsing runtime function). Implements only",
        "// the named behaviors and Action.*(...) references this site",
